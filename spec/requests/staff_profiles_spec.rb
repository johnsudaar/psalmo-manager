...
RSpec.describe "StaffProfiles", type: :request do
  ...
  describe "GET /staff_profiles" do
    ...
    it "renders spreadsheet-style columns and totals" do
      ...
      expect(response.body).to include("Animateur")
      expect(response.body).to include("Pris en charge Psalmodia")
      expect(response.body).to include("Membres à payer")
      expect(response.body).to include("Règlement")
      expect(response.body).to include("bg-white rounded-lg shadow-sm border border-gray-200")
      expect(response.body).not_to include("sticky left-0")
      expect(response.body).not_to include("sticky right-0")
